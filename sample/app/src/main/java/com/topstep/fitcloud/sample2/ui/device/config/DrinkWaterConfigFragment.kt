...
import com.topstep.fitcloud.sample2.databinding.FragmentDrinkWaterConfigBinding
...
/**
 * **Document**
 * https://github.com/htangsmart/FitCloudPro-SDK-Android/wiki/4.Device-info-and-configs#fcdrinkwaterconfig
 *
 * ***Description**
 ...
 * Display and modify
 */
class DrinkWaterConfigFragment : BaseFragment(R.layout.fragment_drink_water_config), CompoundButton.OnCheckedChangeListener,
    ...
    private lateinit var config: FcDrinkWaterConfig

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        config = deviceManager.configFeature.getDrinkWaterConfig()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycle.launchRepeatOnStarted {
            ...
            launch {
                deviceManager.configFeature.observerDrinkWaterConfig().asFlow().collect {
                    if (config != it) {
                        config = it
                        updateUI()
                    }
    ...
    private val blockClick: (View) -> Unit = { view ->
        when (view) {
            ...
            viewBind.itemIntervalTime -> {
                showIntervalDialog(config.getInterval(), 30, 180)
            }
        }
    }
    ...
    private fun FcDrinkWaterConfig.saveConfig() {
        applicationScope.launchWithLog {
            deviceManager.configFeature.setDrinkWaterConfig(this@saveConfig).await()
        }
        this@DrinkWaterConfigFragment.config = this
        updateUI()
    }
