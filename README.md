...
Configure the type of message notification that the bracelet can receive and display. Among them `NotificationConfig.FLAG_EMAIL`, `NotificationConfig.FLAG_TELEGRAM`, `NotificationConfig.FLAG_VIBER` may not be supported in some bracelets, you need to judge the function support in `WristbandVersion`, as follows:
...
SportData{
    ...
    List<SportHR> getHrs();//Heart rate datas
}
```
